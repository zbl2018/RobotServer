...
void net_init(){
    ...
    printf("connect the remote server again...")
    while(is_connect<0){
        is_connect=connect(sockfd,(struct sockaddr *)&serv_addr, sizeof(serv_addr));
    }
    printf("welcome\n"); 
...
void decodeTask(char* s){
    ...
    if(reader.parse(s,value)){
        if(!value["action"].isNull()){
            ...
            cout << "x now: " << tasknode[0].lat << " y now: " << tasknode[0].lon << endl;
            cout << "task x: " << tasknode[1].lat << " task y: " << tasknode[1].lon << endl; 
...
void sendFile(int sockfd,string filename,string action){
    Json::Value root;
    ...
    //     getline(infile_feat,line_s);
    //     stringstream stringin(line_s);
    //     while(stringin>>x>>y){
    //         stringstream ss_stream;//change int to string
    //         string X,Y,Z;

    //         ss_stream<<x;
    //         ss_stream>>X;
    //         map_data["nodeList"][line_id]["x"] = X;
    //         ss_stream.clear();

    //         ss_stream<<y;
    //         ss_stream>>Y;
    //         map_data["nodeList"][line_id]["y"] = Y;
    //         ss_stream.clear();
    ...
    if(!mapreader.parse(infile_feat,mapjson))
    {
    ...
    cout<<"location send successfull"<<endl;
    infile_feat.close();
}
...
void LocationHandler(const std_msgs::String::ConstPtr &msg){
    ...
    if(length_two_points(last_lat,last_lon,lat_now,lon_now) > 8.0){
    	lat_now = last_lat;
    	lon_now = last_lon;
    }
...
void sendPlanner(const char* s){
    ...
    path_log<<"pathplan dyy plan josn:"<<jstr<<endl;
...
void* recData(void* args){
    while(true){
        long recv_length = GetMsgLength(sockfd);//
                ...
                cout<<"the result of recv:"<<msg<<endl<<recv_msg<<endl;
                if(msg==-1||msg==0){
                    cout<<"failed to recv msg"<<endl;
                    closed(sockfd);
                    net_init();
                ...
                record_sss << timestring << " " << sss << endl;
                ...
                if(action_.compare("buildMapStart") == 0){//构建地图
                    ...
                    pathPlan pa;
                    Planner(sss);
                    path_log<<"received planner message: "<<sss<<endl;
                    path_log<<"node: ";
...
int main(int argc, char **argv){
    ...
    /////////////////////////////////////////////   
        ...
        if(((is_car_arrvie == 1) && (cspeed < 0.01))){//length_two_points(task_x,task_y,lat_now,lon_now) < 0.5
            ...
            //cout << "is_car_arrvie : "<< is_car_arrvie <<endl;
